...
final class FottoSync{
 ...
 private static void run(Context c,long after,boolean resetErrors){
  ...
  try{
   ...
   int handled=0;while(handled<40){String[] x=jobs.nextFotto(gallery,after);if(x==null)break;String id=x[0],name=x[1],uri=x[2];
    try{
   ...
   if(handled==0&&resetErrors)p.edit().putString("fottoLastStatus","Nenhuma foto aprovada pendente para este evento.").apply();
 ...
 private static void runSelected(Context c,ArrayList<String> ids){
  ...
  try{
   ...
   for(String id:ids){String[] j=jobs.editorJob(id);if(j==null||!"done".equals(j[5])||j[4]==null||j[4].isEmpty()){skipped++;continue;}if(jobs.fottoWasSent(id,gallery)){skipped++;continue;}String name=j[1].replaceFirst("(?i)\\.jpg$","_Editada.jpg");
   ...
   if(sent>0)p.edit().putString("fottoLastStatus",sent+" foto(s) confirmada(s) no Fotto"+(skipped>0?" · "+skipped+" ignorada(s)":"")).apply();else if(skipped>0)p.edit().putString("fottoLastStatus","As fotos selecionadas já estavam confirmadas ou ainda não estão aprovadas.").apply();
