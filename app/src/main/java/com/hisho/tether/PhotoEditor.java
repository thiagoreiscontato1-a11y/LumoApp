...
final class PhotoEditor {
 ...
 static JSONObject baseControls(File source,JSONObject options)throws Exception{JSONObject a=options.optBoolean("auto",true)?autoControls(source):new JSONObject();double strength=limit(options.optDouble("autoStrength",1),0,1.5);a=scaleControls(a,strength);JSONObject p=presetControls(options.optJSONObject("preset"));return sumControls(a,p);}
 static JSONObject scaleControls(JSONObject a,double strength){JSONObject r=new JSONObject();if(a==null)return r;Iterator<String> it=a.keys();while(it.hasNext()){String k=it.next();try{r.put(k,a.optDouble(k,0)*strength);}catch(Exception ignored){}}return r;}
 static void auto(int[] data){auto(data,1.0);}
 static void auto(int[] data,double strength){
  strength=limit(strength,0,1.5);int[] original=strength==1.0?null:data.clone();
  ...
  if(original!=null){for(int i=0;i<data.length;i++){int a=original[i],b=data[i];double t=strength;int r=(int)Math.round(((a>>16)&255)*(1-t)+((b>>16)&255)*t),g=(int)Math.round(((a>>8)&255)*(1-t)+((b>>8)&255)*t),bl=(int)Math.round((a&255)*(1-t)+(b&255)*t);data[i]=0xff000000|(Math.max(0,Math.min(255,r))<<16)|(Math.max(0,Math.min(255,g))<<8)|Math.max(0,Math.min(255,bl));}}
 }
 ...
 static void edit(File source,File destination,JSONObject options)throws Exception{
  Bitmap bitmap=null;try{
   ...
   int w=bitmap.getWidth(),h=bitmap.getHeight();int[] data=new int[w*h];bitmap.getPixels(data,0,w,0,0,w,h);if(options.optBoolean("auto",true))auto(data,options.optDouble("autoStrength",1.0));JSONObject preset=options.optJSONObject("preset");if(preset!=null)preset(data,w,h,preset);JSONObject manual=options.optJSONObject("manual");if(manual!=null)manual(data,w,h,manual);bitmap.setPixels(data,0,w,0,0,w,h);
