...
final class WorkProfiles{
 ...
 static JSONObject defaults(String n){
  JSONObject j=new JSONObject();try{
   ...
   else if("Ensaio".equals(n)){j.put("curationSensitivity",2);j.put("autoStrength",.55);j.put("backfill",true);}
   else if("Futebol".equals(n)){j.put("curationSensitivity",1);j.put("autoStrength",.9);j.put("backfill",true);j.put("autoUpload",true);}
   else{j.put("curationSensitivity",1);j.put("autoStrength",1.0);j.put("backfill",true);}
 ...
 static void saveCurrent(Context c,String name){
  ...
  JSONObject j=new JSONObject();try{j.put("curationSensitivity",p.getInt("curationSensitivity",1));j.put("autoStrength",Double.longBitsToDouble(p.getLong("autoStrengthBits",Double.doubleToLongBits(1.0))));j.put("backfill",p.getBoolean("backfillEnabled",true));j.put("curation",p.getBoolean("curationEnabled",true));j.put("auto",p.getBoolean("autoEnabled",true));j.put("presetEnabled",p.getBoolean("presetEnabled",true));j.put("presetName",p.getString("presetName","Vibrant"));j.put("preset",p.getString("preset","{}"));j.put("exportTreeUri",p.getString("exportTreeUri",""));root.put(name,j);p.edit().putString("workProfiles",root.toString()).apply();}catch(Exception ignored){}
 }
