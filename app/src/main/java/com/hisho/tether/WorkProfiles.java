...
final class WorkProfiles{
 ...
 static void saveCurrent(Context c,String name){
  ...
  JSONObject j=new JSONObject();try{j.put("curationSensitivity",p.getInt("curationSensitivity",1));j.put("autoStrength",Double.longBitsToDouble(p.getLong("autoStrengthBits",Double.doubleToLongBits(1.0))));j.put("backfill",p.getBoolean("backfillEnabled",true));j.put("autoUpload",false);j.put("curation",p.getBoolean("curationEnabled",true));j.put("auto",p.getBoolean("autoEnabled",true));j.put("presetEnabled",p.getBoolean("presetEnabled",true));j.put("presetName",p.getString("presetName","Vibrant"));j.put("preset",p.getString("preset","{}"));j.put("exportTreeUri",p.getString("exportTreeUri",""));j.put("fottoGalleryId",p.getString("fottoGalleryId",""));j.put("fottoGalleryTitle",p.getString("fottoGalleryTitle",""));root.put(name,j);p.edit().putString("workProfiles",root.toString()).apply();}catch(Exception ignored){}
 }
