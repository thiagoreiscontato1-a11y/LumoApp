...
public class MainActivity extends Activity {
 ...
 String statusSymbol(PhotoItem x){if("review".equals(x.state))return "!";if("confirmed".equals(x.fottoState)||x.sent)return "↑";if("uploading".equals(x.fottoState)||"uploaded".equals(x.fottoState))return "◌";if("unconfirmed".equals(x.fottoState)||"error".equals(x.fottoState))return "⚠";if("done".equals(x.state))return "✓";return "●";}
 int statusColor(PhotoItem x){if("review".equals(x.state))return WARN;if("confirmed".equals(x.fottoState)||x.sent)return 0xff5f6de8;if("unconfirmed".equals(x.fottoState)||"error".equals(x.fottoState))return DANGER;if("uploading".equals(x.fottoState)||"uploaded".equals(x.fottoState))return GREEN;if("done".equals(x.state))return SUCCESS;return GREEN;}
 ...
 void sendSelected(){if(editSelection.isEmpty()){message("Selecione fotos aprovadas para enviar.");return;}if(!checkFottoReady())return;FottoSync.sendSelected(MainActivity.this,new ArrayList<>(editSelection));message("Fotos selecionadas adicionadas à entrega.");}
