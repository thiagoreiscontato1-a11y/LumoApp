...
final class Jobs extends SQLiteOpenHelper{
 ...
 synchronized String[] nextFotto(String galleryId,long after){
  String sql="SELECT j.id,j.name,j.edited,j.rowid FROM jobs j LEFT JOIN fotto_uploads f ON f.job_id=j.id AND f.gallery_id=? WHERE j.state='done' AND j.edited IS NOT NULL AND j.rowid>? AND (f.state IS NULL OR ((f.state='error' OR f.state='unconfirmed') AND f.attempts<4)) ORDER BY j.rowid LIMIT 1";
 ...
 synchronized ArrayList<String[]> fottoNeedsConfirmation(String galleryId){ArrayList<String[]> out=new ArrayList<>();try(Cursor c=getReadableDatabase().rawQuery("SELECT f.job_id,j.name,f.media_id FROM fotto_uploads f JOIN jobs j ON j.id=f.job_id WHERE f.gallery_id=? AND f.state IN ('uploaded','unconfirmed') ORDER BY f.updated LIMIT 40",new String[]{galleryId})){while(c.moveToNext())out.add(new String[]{c.getString(0),c.getString(1).replaceFirst("(?i)\\.jpg$","_Editada.jpg"),c.getString(2)});}return out;}
 ...
 synchronized void fottoRetry(String galleryId){ContentValues v=new ContentValues();v.put("attempts",0);v.put("state","error");getWritableDatabase().update("fotto_uploads",v,"gallery_id=? AND state IN ('error','unconfirmed')",new String[]{galleryId});}
