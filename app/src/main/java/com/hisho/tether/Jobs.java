...
final class Jobs extends SQLiteOpenHelper{
 ...
 public void onCreate(SQLiteDatabase d){d.execSQL("CREATE TABLE jobs (id TEXT PRIMARY KEY, file TEXT NOT NULL, name TEXT NOT NULL, settings TEXT NOT NULL, original TEXT, edited TEXT, state TEXT NOT NULL, error TEXT, score INTEGER NOT NULL DEFAULT 0)");createFotto(d);}
 public void onUpgrade(SQLiteDatabase d,int a,int b){if(a<2)createFotto(d);if(a<3)try{d.execSQL("ALTER TABLE jobs ADD COLUMN score INTEGER NOT NULL DEFAULT 0");}catch(Exception ignored){}}
 ...
 synchronized void setScore(String id,int score){ContentValues v=new ContentValues();v.put("score",Math.max(0,Math.min(10,score)));getWritableDatabase().update("jobs",v,"id=?",new String[]{id});}
 synchronized int score(String id){try(Cursor c=getReadableDatabase().rawQuery("SELECT score FROM jobs WHERE id=?",new String[]{id})){return c.moveToFirst()?c.getInt(0):0;}}
 ...
 synchronized int processedCount(){try(Cursor c=getReadableDatabase().rawQuery("SELECT count(*) FROM jobs WHERE state IN ('done','review')",null)){c.moveToFirst();return c.getInt(0);}}
 ...
 synchronized double averageScore(){try(Cursor c=getReadableDatabase().rawQuery("SELECT COALESCE(AVG(score),0) FROM jobs WHERE state IN ('done','review') AND score>0",null)){c.moveToFirst();return c.getDouble(0);}}
 ...
 synchronized void fottoRetryOne(String jobId,String galleryId){ContentValues v=new ContentValues();v.put("attempts",0);v.put("state","error");v.put("error","");v.put("updated",System.currentTimeMillis());int n=getWritableDatabase().update("fotto_uploads",v,"job_id=? AND gallery_id=?",new String[]{jobId,galleryId});if(n==0)fottoSet(jobId,galleryId,"error","","",false);}
 ...
 synchronized int[] fottoStats(String galleryId){int done=0,uploading=0,pending=0,error=0;if(galleryId==null||galleryId.isEmpty())return new int[]{0,0,0,0};String sql="SELECT COALESCE(f.state,'pending'),COUNT(*) FROM jobs j LEFT JOIN fotto_uploads f ON f.job_id=j.id AND f.gallery_id=? WHERE j.state='done' AND j.edited IS NOT NULL GROUP BY COALESCE(f.state,'pending')";try(Cursor c=getReadableDatabase().rawQuery(sql,new String[]{galleryId})){while(c.moveToNext()){String s=c.getString(0);int n=c.getInt(1);if("done".equals(s))done=n;else if("uploading".equals(s))uploading=n;else if("error".equals(s))error=n;else pending+=n;}}return new int[]{done,uploading,pending,error};}
