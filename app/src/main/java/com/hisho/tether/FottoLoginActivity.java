...
public class FottoLoginActivity extends Activity{
 ...
 void deliver(String key,String origin,boolean bearerWorked){if(delivered||destroyed)return;delivered=true;handler.removeCallbacks(retry);String token=clean(key);String api=clean(seenApiKey),access=clean(seenAccessToken);if(api.isEmpty()&&!bearerWorked)api=token;if(access.isEmpty()&&bearerWorked)access=token;if(api.isEmpty())api=token;if(access.isEmpty())access=token;getSharedPreferences("hisho",0).edit().putString("fottoApiKeyV7",api).putString("fottoAccessTokenV7",access).remove("fottoApiKeyV5").remove("fottoAccessTokenV5").remove("fottoApiKeyV4").remove("fottoAccessTokenV4").remove("fottoToken").remove("fottoTokenV2").remove("fottoTokenV3").putString("fottoLastStatus","Fotto conectado ao LUMO. Carregando eventos…").apply();status.setText("Conta conectada. Voltando ao LUMO…");Intent data=new Intent().putExtra("fottoToken",token).putExtra("fottoOrigin",origin).putExtra("fottoBearer",bearerWorked);setResult(RESULT_OK,data);handler.postDelayed(()->finish(),180);}
