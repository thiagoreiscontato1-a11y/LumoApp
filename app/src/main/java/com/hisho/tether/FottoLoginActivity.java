...
public class FottoLoginActivity extends Activity{
 ...
 WebView web;TextView status;Button capture;boolean delivered=false,destroyed=false;int attempts=0;final Handler handler=new Handler(Looper.getMainLooper());final ExecutorService net=Executors.newSingleThreadExecutor();final Set<String> tested=Collections.synchronizedSet(new HashSet<String>());
 ...
 void setup(){WebSettings s=web.getSettings();s.setJavaScriptEnabled(true);s.setDomStorageEnabled(true);s.setDatabaseEnabled(true);s.setJavaScriptCanOpenWindowsAutomatically(true);s.setUserAgentString(s.getUserAgentString()+" LUMO/0.9.5");
 ...
 void parseAndMaybeDeliver(String value){try{Object outer=new JSONTokener(value==null?"{}":value).nextValue();String raw=outer instanceof String?(String)outer:String.valueOf(outer);if(raw.startsWith("\"")&&raw.endsWith("\"")&&raw.length()>1)raw=new JSONTokener(raw).nextValue().toString();JSONObject o=new JSONObject(raw);ArrayList<String> candidates=new ArrayList<>();collectPreferred(o,candidates);for(String v:candidates)if(candidate(v))tryCandidate(v,"storage");}catch(Exception ignored){}}
 ...
 void tryCandidate(String raw,String origin){final String key=clean(raw);if(!candidate(key)||delivered||!tested.add(key))return;status.setText("Validando sessão Fotto…");net.execute(()->{try{FottoApi.callWithToken("GET","/me",null,key);runOnUiThread(()->deliver(key,origin));}catch(Exception ignored){}});}

 void deliver(String key,String origin){if(delivered||destroyed)return;delivered=true;handler.removeCallbacks(retry);String token=clean(key);getSharedPreferences("hisho",0).edit().putString("fottoApiKeyV5",token).putString("fottoAccessTokenV5",token).remove("fottoApiKeyV4").remove("fottoAccessTokenV4").remove("fottoToken").remove("fottoTokenV2").remove("fottoTokenV3").putString("fottoLastStatus","Fotto conectado ao LUMO. Carregando eventos…").apply();status.setText("Conta conectada. Voltando ao LUMO…");Intent data=new Intent().putExtra("fottoToken",token).putExtra("fottoOrigin",origin);setResult(RESULT_OK,data);handler.postDelayed(()->finish(),180);}
